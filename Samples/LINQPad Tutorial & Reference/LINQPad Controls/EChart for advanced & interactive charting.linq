...
var dc = new DumpContainer (Util.Highlight ("Click a data point...\n\nDouble-click to randomize"));

var interactiveChart = new EChart (
	new
	{
		title = new { text = "Interactive Chart" },
		xAxis = new { type = "category", data = new[] { "A", "B", "C", "D", "E" } },
		yAxis = new { type = "value" },
		series = new[] { new { type = "bar", data = GetRandomNumbers (5) } }
	},
...
new WrapPanel (childVerticalAlign: "middle", children: [interactiveChart, dc]).Dump ();
...
lineChart.GetSvgAsync ().Dump ("SVG for lineChart");
